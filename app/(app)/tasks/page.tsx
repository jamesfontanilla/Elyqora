...
export default async function TasksPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const pageData = await getTasksListPageData({ userId: user.id, mode: "all", search, page, label });
  if (!pageData) return null;
  return <TasksLibrary workspace={pageData.workspace} data={pageData.data} mode="all" search={search} basePath="/tasks" label={label} canWrite={pageData.canWrite} canManage={pageData.canManage} members={pageData.members} />;
}
