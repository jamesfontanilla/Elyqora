...
export default async function PinnedNotesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [data, canWrite] = await Promise.all([
    getNotesList({ workspaceId: workspace.id, search, page, mode: "pinned" }),
    hasPermission(workspace.id, "notes.write"),
  ]);

  return <NotesLibrary workspace={workspace} data={data} mode="pinned" search={search} basePath="/notes/pinned" canWrite={canWrite} />;
}
