...
export default async function ArchivedNotesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [data, canWrite] = await Promise.all([
    getNotesList({ workspaceId: workspace.id, search, page, mode: "archived" }),
    hasPermission(workspace.id, "notes.write"),
  ]);

  return <NotesLibrary workspace={workspace} data={data} mode="archived" search={search} basePath="/notes/archived" canWrite={canWrite} />;
}
